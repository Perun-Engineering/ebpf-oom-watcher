...
impl KubernetesClient {
    pub async fn new() -> Result<Self> {
        let config = Config::incluster_default().await
    ...
    pub async fn get_container_info(&self, pid: u32) -> Result<Option<(String, String, String, String)>> {
        let container_id = self.get_container_id_from_pid(pid)?;
        
        if let Some(container_id) = container_id {
            if let Some((namespace, pod_name, container_name)) = self.get_pod_info_from_container_id(&container_id).await? {
    ...
    fn get_container_id_from_pid(&self, pid: u32) -> Result<Option<String>> {
        ...
        debug!("Could not extract container ID from cgroup for PID {}: {}", pid, content);
        Ok(None)
    }

    async fn get_pod_info_from_container_id(&self, container_id: &str) -> Result<Option<(String, String, String)>> {
                ...
                if let Some(container_statuses) = &status.container_statuses {
                    for container_status in container_statuses {
                        if let Some(container_id_full) = &container_status.container_id {
                            ...
                            if container_id_full.ends_with(container_id) || container_id_full.contains(container_id) {
