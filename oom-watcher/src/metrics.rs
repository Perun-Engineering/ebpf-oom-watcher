...
impl MetricsCollector {
    pub fn new() -> Self {
        ...
        let oom_kills_total = CounterVec::new(
            prometheus::Opts::new(
                "oom_kills_total",
                "Total number of OOM kills observed"
            ),
        ...
        let oom_last_timestamp = GaugeVec::new(
            prometheus::Opts::new(
                "oom_last_timestamp",
                "Timestamp of the last OOM kill event"
            ),
            &["node", "namespace", "pod", "container"]
        ).expect("Failed to create oom_last_timestamp metric");

        registry.register(Box::new(oom_kills_total.clone()))
            .expect("Failed to register oom_kills_total");
        registry.register(Box::new(oom_kills_per_node_total.clone()))
            .expect("Failed to register oom_kills_per_node_total");
        registry.register(Box::new(oom_memory_usage_bytes.clone()))
            .expect("Failed to register oom_memory_usage_bytes");
        registry.register(Box::new(oom_last_timestamp.clone()))
...
async fn metrics_handler(State(collector): State<Arc<MetricsCollector>>) -> Result<Response<String>, StatusCode> {
